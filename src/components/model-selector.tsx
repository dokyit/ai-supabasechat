...
export default function ModelSelector({ onChange }: { onChange: (m: string) => void }) {
  ...
  const [selected, setSelected] = useState<string>('llama3.1:latest');
  const [provider, setProvider] = useState<'local' | 'openai' | 'gemini'>('local');

  useEffect(() => { fetch('/api/models/local').then(r => r.json()).then(setModels); }, []);

  const handle = (v: string) => {
    setSelected(v);
    if (v.startsWith('remote::')) { setOpen(true); } else { onChange(v); }
  };

  return (
    <>
      <Select value={selected} onValueChange={handle}>
        <SelectTrigger className="w-48"><SelectValue placeholder="llama3.1:latest" /></SelectTrigger>
        <SelectContent>
          <SelectItem value="local" disabled>Local Models</SelectItem>
          {models.map(m => <SelectItem key={m} value={m}>{m}</SelectItem>)}
          <SelectItem value="remote-label" disabled>Remote (needs key)</SelectItem>
          {REMOTE_MODELS.map(m => (
            <SelectItem key={m.value} value={m.value} disabled={m.value === 'remote::openrouter::' ? false : undefined}>
              {m.label}
            </SelectItem>
          ))}
        </SelectContent>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Provide API key</DialogTitle></DialogHeader>
